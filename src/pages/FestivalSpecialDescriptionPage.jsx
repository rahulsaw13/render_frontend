...
const FestivalSpecialDescriptionPage = () => {
  ...
  const festName = location?.state?.fest_name || {};
  ...
  const getFestivalSpecialProducts = () =>{
    return allApi(API_CONSTANTS.GET_PRODUCT_BY_FESTIVAL_SPECIAL_NAME_URL, "", "get")
    .then((response) => {
      ...
      <>
        ...
        <div className="bg-[#fdfaf2] text-center py-8 mt-16">
            ...
            <nav className="text-gray-600 text-[16px]">
                <span className='hover:cursor-pointer' onClick={()=>{ navigate("/") }}>{t("home")}</span> <span className='px-4'>&gt;</span> <span className="text-gray-600 hover:cursor-pointer">{festName}</span>
            </nav>
        ...
        <div className="bg-[#fdfaf2] py-8 px-4">
            <div className="grid grid-cols-3 md:grid-cols-6 gap-6">
                {products?.subcategories?.map((item, index) => (
...
export default FestivalSpecialDescriptionPage;